...
def validate_submission(csv_path):
    ...
    if n != EXPECTED_DATA_ROWS:
        errors.append(
            "After the header, there must be exactly " +
            str(EXPECTED_DATA_ROWS) +
            " data rows; found " + str(n) + "."
        )
    ...
    for i, cells in enumerate(data_rows):
        ...
        if len(cells) != len(REQUIRED_HEADER):
            errors.append("Row " + str(row_num) + ": expected 4 columns, got " + str(len(cells)) + ".")
            continue
        ...
        if not cid:
            errors.append("Row " + str(row_num) + ": candidate_id is required.")
        elif not CANDIDATE_ID_PATTERN.match(cid):
            errors.append("Row " + str(row_num) + ": candidate_id must be CAND_XXXXXXX (7 digits).")
        elif cid in seen_ids:
            errors.append("Row " + str(row_num) + ": duplicate candidate_id " + repr(cid) + ".")
        else:
            seen_ids.add(cid)
        ...
        try:
            rank = int(rank_s)
            if not 1 <= rank <= 100:
                errors.append("Row " + str(row_num) + ": rank must be between 1 and 100.")
            elif rank in seen_ranks:
                errors.append("Row " + str(row_num) + ": duplicate rank " + str(rank) + ".")
            else:
                seen_ranks.add(rank)
        except ValueError:
            errors.append("Row " + str(row_num) + ": rank must be an integer (1-100).")
            rank = None

        try:
            score = float(score_s)
        except ValueError:
            errors.append("Row " + str(row_num) + ": score must be a float.")
            score = None
    ...
    if missing:
        errors.append("Missing ranks: " + str(sorted(missing)))
    ...
    for i in range(len(by_rank) - 1):
        ...
        if s1 < s2:
            errors.append(
                "score must be non-increasing by rank: rank " +
                str(r1) + " (" + str(s1) + ") < rank " +
                str(r2) + " (" + str(s2) + ")."
            )
    ...
    for i in range(len(by_rank) - 1):
        ...
        if s1 == s2 and c1 > c2:
            errors.append(
                "Equal scores at ranks " + str(r1) + " and " + str(r2) +
                ": tie-break requires candidate_id ascending (" +
                repr(c1) + " > " + repr(c2) + ")."
            )
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python validate_submission.py <file>.csv")
        sys.exit(1)

    errors = validate_submission(sys.argv[1])
    if errors:
        print("Validation failed (" + str(len(errors)) + " issue(s)):")
        for e in errors:
            print("- " + e)
        sys.exit(1)
